...
//Moves up to 4 people from pool into tile, adjsuts tile.population
double City::distributePool(double& pool, Tile& tile, double rate = 0.0)
        //rate > 0 birth rate,< death rate
{
...
void City::shuffleTiles()
{
    ...
    //std::random_shuffle(shuffledTiles.begin(), shuffledTiles.end()); deprecated on c++17
    std::random_device rd;
    std::default_random_engine eng(rd());
    std::shuffle(shuffledTiles.begin(), shuffledTiles.end(), eng);
...
void City::update(float dt)
{
    ...
    //Move onto next day if enough time has passed
    this->currentTime += dt;
    ...
    if(day % 30 == 0)
    {
        //update spending funds from pool
        this->funds += this->earnings;
        this->earnings = 0;
    }

    //Iterate over each tile (shuffled tiles)
    //Seperate loop for each logic (denotes order)

    /* Run first pass of tile updates. Mostly handles pool distribution */
    //People moving from pools into residential zone
    for(int i = 0; i < this->map.tiles.size(); ++i)
    {
        ...
        if(tile.tileType == TileType::RESIDENTIAL)
        {
            ...
            //NOTE: This will grow the population of a tile depending on rate
            //If the new population > maxPop, then the remainder is moved into population pool
            //rabbits smh
    ...
    //Goods prpduced by industrial moved to commercial
    //commercial then sells goods
    //connectedness increases production & sales
    for(int i = 0; i < this->map.tiles.size(); ++i)
    {
        Tile& tile = this->map.tiles[this->shuffledTiles[i]];
    ...
    this->populationPool += this->populationPool*growthRate;
