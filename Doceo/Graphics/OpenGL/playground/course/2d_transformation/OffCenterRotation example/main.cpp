...
int main()
{
    ...
    Shape* shp = new Square(vec2(-0.75, 0.25), vec2(-0.75, 0.75), vec2(-0.25, 0.75), vec2(-0.25, 0.25), vec3(0,0,1));
    vector<vec3> colours;

    //two identity matrices
    mat3 model = mat3(1.0f);
    mat3 MVP = MVPMatrixCreator();
    ...
    do
    {
        ...
        vector<vec2> worldVertices = applyMatrix(shp->getVertices(), MVP);
        GLfloat *vertices = toGLfloat(worldVertices);
        GLfloat *colors = toGLfloat(shp->getColours());
        ...
        float rotationalAngle = 0.01;
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        {
            vec2 center = shp->getCenter();
            mat3 translationA = mat3(1.0f);
            translationA[2] = vec3(center* -1.0f, 1) ; //Move to origin

            mat3 rotation = mat3(1.0f);
            rotation[0] = vec3(cos(rotationalAngle), sin(rotationalAngle), 0);
            rotation[1] = vec3(-1*sin(rotationalAngle), cos(rotationalAngle), 0);

            mat3 translationB = mat3(1.0f);
            translationB[2] = vec3(center, 1); //Move to original place
            mat3 T = translationB * rotation * translationA;
            model = model * T;
            MVP = MVPMatrixCreator(model);
        }
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
        {
            ...
            mat3 rotation = mat3(1.0f);
            rotation[0] = vec3(cos(-1.0f*rotationalAngle), sin(-1.0f*rotationalAngle), 0);
            rotation[1] = vec3(-1*sin(-1.0f*rotationalAngle), cos(-1.0f*rotationalAngle), 0);
