...
public:
    Shape2D(unsigned int sid, unsigned int vao) : Shape(sid, vao){}
...
public:
    Triangle(unsigned int sid, unsigned int vao, float points[9]) : Shape2D(sid, vao) {
        vertices.resize(9);
        for (int i = 0; i < 9; i++) {
            vertices[i] = points[i];
        }
...
public:
    Square(unsigned int sid, unsigned int vao, float points[3], float length) : Shape2D(sid, vao) {
        float x2 = points[0];
        float y2 = points[1];
        float x1 = x2 - length;
        float y1 = y2 - length;
...
class Rectangle : public Shape2D {
    ...
    Rectangle(unsigned int sid, unsigned int vao, float points[12]) : Shape2D(sid, vao) {
        vertices = {
    ...
    Rectangle(unsigned int sid, unsigned int vao, Vector<12> points) : Shape2D(sid, vao) {
        vertices = {
...
public:
    Circle(unsigned int sid, unsigned int vao, float radius, float center_x, float center_y, int segment_count = 32)
        : Shape2D(sid, vao), radius(radius), center_x(center_x), center_y(center_y), segment_count(segment_count < 3 ? 3 : segment_count) {
        generateVertices();
    }
