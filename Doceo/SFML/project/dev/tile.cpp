...
void Tile::draw(sf::RenderWindow& window, float dt)
{
    ...
    this->animHandler.changeAnim(this->tileVariant);
        //tileVariant cooresponds to the animation (y pos)
        //Allows us to keep all sprites in one file
        //Then just change the y pos depending on which tile we want (animations run rightward)
    ...
    this->sprite.setTextureRect(this->animHandler.bounds);
        //create bounds
        //i.e) location of frame in texture 
