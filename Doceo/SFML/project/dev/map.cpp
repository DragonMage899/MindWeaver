    ...
    std::map<std::string, Tile>& tileAtlas)
{
    ...
    inputFile.open(filename, std::ios::in | std::ios::binary); //binary file

    this->width = width;
    this->height = height;

    for(int pos = 0; pos < this->width * this->height; ++pos) //map is a vector, similar to how a 2d array is stored in memory
    {
        this->resources.push_back(255);
        this->selected.push_back(0);

        TileType tileType;
        inputFile.read((char*)&tileType, sizeof(int)); //char* -> start of byte array to write into, sizeof -> number of bytes to read
        switch(tileType)
        {
            default:
            case TileType::VOID:
