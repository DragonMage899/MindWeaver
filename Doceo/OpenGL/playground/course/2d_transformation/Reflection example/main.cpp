...
int main()
{
    ...
    float timeSinceLastPressed = glfwGetTime();
    ...
    do
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_MINUS) == GLFW_PRESS && abs(timeSinceLastPressed - currentTime) >= 1)
        {
            //Here we set the identity matrix
            mat3 scale = mat3(1.0f);
            //Reminder: GLM uses column major matrices and vectors. So here we are setting the first and second column in the matrix not the row!
            scale[0] = vec3(-1,0,0);
            scale[1] = vec3(0,1,0);
            model = model * scale;
            MVP = MVPMatrixCreator(model);
            timeSinceLastPressed = glfwGetTime();
        }
        if (glfwGetKey(window, GLFW_KEY_EQUAL) == GLFW_PRESS && abs(timeSinceLastPressed - currentTime) >= 1)
        {
            mat3 scale = mat3(1.0f);
            scale[0] = vec3(1,0,0);
            scale[1] = vec3(0,-1,0);
        ...
        cout << abs(timeSinceLastPressed - currentTime) << endl;
